    ...
    def load_block_with_metadata(
        ...
        if entry is not None:
            ...
            if self._hot_cache_enabled and promote_to_hot_cache:
                # Keep the same raw-byte objects so the writer can mark this
                # promoted entry clean after the pending SSD write succeeds.
                self._hot_cache_put(block_hash, dict(entry))
                self._stats["hot_cache_promotions"] += 1
            logger.debug(
