...
class Scheduler:
    ...
    def _step_prefill_chunk(self, state: _PrefillState) -> bool:
        ...
        if (
            self._should_clear_after_chunk()
            and mx.get_cache_memory() > self._periodic_clear_threshold_bytes()
        ):
            _sync_and_clear_cache(self._stream)
        ...
        while (
            ...
            with mx.stream(self._stream):
                uids = self.batch_generator.insert(
                    ...
                    all_tokens=[_batch_generator_all_tokens(request)],
                    # fork: None when params match the fallback sampler
                    # (batched sampling fast path; see _sampler_for_insert).
